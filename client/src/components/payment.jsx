...
function Payment() {
  ...
  const handleSearch = async () => {
    setMessage("");
    try {
      const res = await fetch('http://localhost:3001/parking',{
        method:"POST",
        headers:{
      ...
      if (!res.ok) {
        setMessage(data.message||"Vehicle not found or already left.");
        return;
      }
      else{
         setMessage("Vehicle found");
         setBillDetails({
            amount: data.details.total_fee,
            entry_time: data.details.entry_time
        });
      }
    } catch (err) {
      console.log(err);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {billDetails && (
          <div style={{ borderTop: '2px dashed #ccc', paddingTop: '20px',color:'black' }}>
